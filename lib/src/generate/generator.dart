...
import 'method_generator_config.dart';
import 'parser_expression.dart';
...
class NetRetrofitGenerator extends GeneratorForAnnotation<NetApi> {
  ...
  String _buildUrl(MethodGeneratorConfig config) {
    ...
    final ck = config.clientKey;
    if (ck != null && ck.isNotEmpty) {
      return "url: '\${NetRequest.resolveBaseUrl('$ck')}$pathStr'";
    }
    return "url: '\${NetRequest.resolveBaseUrl()}$pathStr'";
  }
  ...
  /// Builds clientKey expression. If null, it is omitted (default [_dio] stack).
  ...
  bool _returnsDioResponse(DartType type) {
    final t = _unwrapFutureType(type);
    if (t is! InterfaceType || t.element.name != 'Response') return false;
    final uri = t.element.library.uri.toString();
  ...
  String? _buildAutoListReturnStatement(
    ...
    final sourceExpr = config.parserConfig.dataPath != null
        ? '(response.data as Map<String, dynamic>)["${config.parserConfig.dataPath}"]'
        : 'response.data';
  ...
  String _buildReturnStatement(
    ...
    final autoList = _buildAutoListReturnStatement(method, config);
    if (autoList != null) return autoList;
    return buildReturnStatementFromResponse(
      config.parserConfig.returnTypeName,
      config.parserConfig.dataPath,
    );
  }

  /// Emits `NetRequest.request<dynamic>(...)`; [@StreamResponse] adds
  /// `responseType: ResponseType.stream`.
  void _writeNetRequestInvocation(
    StringBuffer buffer,
    MethodElement method,
    MethodGeneratorConfig config, {
    ...
    if (streamResponse) {
      buffer.writeln('      responseType: ResponseType.stream,');
    }
  ...
  String _generateMethodImplementation(
    ...
    _writeNetRequestInvocation(
      buffer,
      method,
      config,
      streamResponse: config.stream,
    );
    if (config.stream) {
